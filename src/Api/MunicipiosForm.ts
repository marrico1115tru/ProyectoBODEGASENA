...
export const createMunicipio = async (municipio: Municipio): Promise<Municipio> => {
  const response = await axios.post(API_URL, municipio);
  return response.data;
};

export const updateMunicipio = async (id: number, municipio: Municipio): Promise<Municipio> => {
